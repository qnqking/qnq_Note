...
你会保存多个版本，以防写坏了还能找回。  
这个“保存历史版本”的思路，就是最原始的版本控制。

Git 只是把这件事**自动化、系统化**了。
...
- 安装 **Git**
- 安装 **VS Code**
- 配置一个 AI 编程工具（例如 Codex、Cloud Code）
- 注册并登录 GitHub 账号
...
- 执行安装命令安装命令行工具
- 里面会包含 Git
- 输入 `git --version` 能看到版本号，就说明安装成功

---

## VS Code 怎么安装

去 VS Code 官网下载对应系统版本，安装即可。  
它是 Git 开发中非常常用的编辑器。
